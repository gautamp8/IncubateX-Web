 ...
 */
friendlyPix.Uploader = function () {_createClass(_class, null, [{ key: 'FULL_IMAGE_SPECS', 
  ...
  function _class() {var _this = this;_classCallCheck(this, _class);
    ...
    $(document).ready(function () {
      ...
      _this.imageInput = $('#fp-mediacapture');
      _this.overlay = $('.fp-overlay', '#page-add');
      _this.newPictureContainer = $('#newPictureContainer');
      _this.uploadButton = $('.fp-upload');
  ...
  _createClass(_class, [{ key: 'addPolyfills', value: function addPolyfills() {
     ...
     */value: function generateImages() 
    {
      ...
      var displayPicture = function displayPicture(url) {
        ...
        // Generate thumb.
        var maxThumbDimension = friendlyPix.Uploader.THUMB_IMAGE_SPECS.maxDimension;
        var thumbCanvas = friendlyPix.Uploader._getScaledCanvas(image, maxThumbDimension);
        thumbCanvas.toBlob(resolveThumbBlob, 'image/jpeg', friendlyPix.Uploader.THUMB_IMAGE_SPECS.quality);

        // Generate full sized image.
        var maxFullDimension = friendlyPix.Uploader.FULL_IMAGE_SPECS.maxDimension;
        var fullCanvas = friendlyPix.Uploader._getScaledCanvas(image, maxFullDimension);
    ...
    e) {var _this3 = this;
      ...
      this.generateImages().then(function (pics) {
        ...
        friendlyPix.firebase.uploadNewPic(pics.full, pics.thumb, _this3.currentFile.name, imageCaption).
     ...
     */ }, { key: 'clear', value: function clear() 
    {
      ...
      this.disableUploadUi(false);} }], [{ key: '_getScaledCanvas', value: function _getScaledCanvas(image, maxDimension) {var thumbCanvas = document.createElement('canvas');if (image.width > maxDimension || image.height > maxDimension) {if (image.width > image.height) {thumbCanvas.width = maxDimension;thumbCanvas.height = maxDimension * image.height / image.width;} else {thumbCanvas.width = maxDimension * image.width / image.height;thumbCanvas.height = maxDimension;}} else {thumbCanvas.width = image.width;thumbCanvas.height = image.height;}thumbCanvas.getContext('2d').drawImage(image, 0, 0, image.width, image.height, 0, 0, thumbCanvas.width, thumbCanvas.height);return thumbCanvas;} }]);return _class;}();
